...
### Rule 6 — 不设 token 预算上限（2026-06-06 用户指令变更）
- **无单任务/单会话 token 硬上限。不因预算剩余比例暂停、收尾或强制停止。**
- 停止条件只看实质：任务真正完成且验证通过、遇到真实 blocker、或用户喊停。预算不再是停止条件。
- 仍按 Rule 10 在关键步骤 checkpoint、更新 `claude-progress.md`、报告进度——这是跨会话连续性需要，与预算无关。
- 自治模式：持续工作直到所有功能 passing 或触发**非预算类**升级条件。
- 历史背景：原 Rule 6 设单任务 4,000 / 单会话 30,000 token 硬上限并按 30%/20% 门控暂停；用户 2026-06-06 明确"忽略所有会话预算限制"，故移除。
...
> 注：预算类升级条件已随 Rule 6（2026-06-06）移除——不再因 token 预算停止自治。
